 ...
 * @author ASUS
 */
@WebServlet(name = "employeeoptions", urlPatterns = {"/employeeoptions.do"})
public class employeeoptions extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");
         ...
         //   ps.write(request.getParameter("select"));
          //  ps.flush();
            if(request.getParameter("opt").equalsIgnoreCase("employee")==true) {
                DataAccess db=new DataAccess();
                  int id=db.getLatestEmployee();   
           ...
           // session.setAttribute("username", username);
            ...
            else if(request.getParameter("opt").equalsIgnoreCase("delete")==true){
                     RequestDispatcher rd = request.getRequestDispatcher("deleteemployee.jsp");
                     rd.forward(request, response);
            }
            else if(request.getParameter("opt").equalsIgnoreCase("view")==true) {
                    DataAccess db=new DataAccess();
                     List<Employee> SelectedRows=db.viewEmployee();
                     HttpSession session = request.getSession();
            ...
            /*
                    ...
                    DataAccess db=new DataAccess();
                    List<String> SelectedRows=db.viewProduct();
